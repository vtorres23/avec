d3.csv(taxa_bruta_natalidade_br_brics_al, function(csv) {
  
  
  var
  // The color scale
  scale = colorbrewer.Oranges[9],

  
  // The SVG container
  width = document.getElementById('correlatos').offsetWidth - (document.getElementById('correlatos').offsetWidth*0.05),
  height = width*0.35,
  chart  = d3.select("#chartmundo")
             .append("svg")
               .attr("width",  width)
               .attr("height", height),
 ...
 // year     = top.location.search.replace(/\?/, "") || min_year,
  ...
  data     = function() {
               return csv
                 .reduce( function(previous, current, index) {
                   previous[ current["Country Code"] ] = parseFloat(current[year || min_year]);
                   return previous;
                 }, {})
              },
  ...
  // Tooltip
    var tooltip = d3.select("#chartmundo")
                    .append("div")
                    .attr("class", "tooltip");
